...
const ExcelImportModal = ({ 
  ...
  const {
    ...
    getFieldLabel,
    stats
  } = useExcelImport(existingCustomers);
          ...
          {activeTab === 'upload' && (
            <div className="space-y-6">
              <div className="bg-blue-50 p-4 rounded-lg">
                <h3 className="font-medium text-blue-800 mb-2">📋 Excel取り込み手順</h3>
            ...
            <div className="space-y-6">
              ...
              <div className="grid grid-cols-4 gap-4">
                <div className="bg-blue-50 p-4 rounded-lg text-center">
                  ...
                  <div className="text-sm text-blue-800">総データ数</div>
                </div>
                <div className="bg-green-50 p-4 rounded-lg text-center">
                  <div className="text-2xl font-bold text-green-600">{stats.validRows}</div>
                  <div className="text-sm text-green-800">有効データ</div>
                </div>
                  ...
                  <div className="max-h-32 overflow-y-auto space-y-1">
                    {duplicateInfo.slice(0, 3).map((dup, index) => (
                      <div key={index} className="text-sm text-orange-700">
                        行{dup.row}: {dup.companyName} (コード: {dup.customerCode}) は既に登録されています
                      </div>
                    ))}
                  ...
                  <table className="w-full text-sm">
                    <thead className="bg-gray-50">
                      <tr>
                        ...
                        <th className="px-3 py-2 text-left font-medium text-gray-700">取引先コード</th>
                        <th className="px-3 py-2 text-left font-medium text-gray-700">会社名</th>
                        <th className="px-3 py-2 text-left font-medium text-gray-700">住所</th>
                        <th className="px-3 py-2 text-left font-medium text-gray-700">電話番号</th>
                        <th className="px-3 py-2 text-left font-medium text-gray-700">FAX</th>
                      </tr>
                    </thead>
                    <tbody>
                      {previewData.map((item, index) => {
                        ...
                        return (
                          ...
                          >
                            ...
                            <td className="px-3 py-2 font-mono text-xs">{item.customerCode}</td>
                            <td className="px-3 py-2 font-medium">{item.companyName}</td>
                            <td className="px-3 py-2 text-xs max-w-48 truncate">{item.address || '-'}</td>
                            <td className="px-3 py-2 text-xs">{item.phone || '-'}</td>
                            <td className="px-3 py-2 text-xs">{item.fax || '-'}</td>
                          </tr>
                        );
                      })}
                    </tbody>
            ...
            <div className="space-y-6">
              <div className="bg-gray-50 p-4 rounded-lg">
                ...
                <p className="text-sm text-gray-600">
                  Excelの列と取引先データの項目を対応付けてください。自動検出された設定を手動で調整できます。
                </p>
              </div>
                    ...
                    return (
                      <div
                        ...
                        className={`p-2 rounded text-sm border ${
                          isUsed ? 'border-blue-300 bg-blue-50 text-blue-800' : 'border-gray-300 bg-gray-50'
                        }`}
                      >
